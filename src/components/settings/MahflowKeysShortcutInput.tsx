...
interface MahflowKeysShortcutInputProps {
...
export const MahflowKeysShortcutInput: React.FC<MahflowKeysShortcutInputProps> = ({
  ...
  const cancelRecording = useCallback(async () => {
    ...
    await commands.stopMahflowKeysRecording().catch(console.error);
  ...
  // Set up event listener for mahflow-keys events
  useEffect(() => {
    if (!isRecording) return;
    ...
    const setupListener = async () => {
      ...
      const unlisten = await listen<MahflowKeysEvent>(
        "mahflow-keys-event",
        async (event) => {
          if (cleanup) return;
    ...
    return () => {
      ...
      commands.stopMahflowKeysRecording().catch(console.error);
    };
  }, [
    isRecording,
    shortcutId,
  ...
  const startRecording = async () => {
    ...
    try {
      await commands.startMahflowKeysRecording(shortcutId);
      setIsRecording(true);
      setCurrentKeys("");
      currentKeysRef.current = "";
    } catch (error) {
