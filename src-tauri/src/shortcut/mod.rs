...
//! - `mahflow_keys`: Uses the mahflow-keys library for more control
//!
//! The active implementation is determined by the `keyboard_implementation`
//! setting and can be changed at runtime.

mod handler;
pub mod mahflow_keys;
mod tauri_impl;
...
// Note: Commands are accessed via shortcut::mahflow_keys:: in lib.rs
...
pub fn init_shortcuts(app: &AppHandle) {
    ...
    match user_settings.keyboard_implementation {
        ...
        KeyboardImplementation::MahflowKeys => {
            if let Err(e) = mahflow_keys::init_shortcuts(app) {
                error!("Failed to initialize mahflow-keys shortcuts: {}", e);
                ...
                // Update settings to persist the fallback so we don't retry MahflowKeys on next launch
...
pub fn register_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::MahflowKeys => mahflow_keys::register_cancel_shortcut(app),
    }
}
...
pub fn unregister_cancel_shortcut(app: &AppHandle) {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::MahflowKeys => mahflow_keys::unregister_cancel_shortcut(app),
    }
}
...
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::MahflowKeys => mahflow_keys::register_shortcut(app, binding),
    }
}
...
pub fn unregister_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::MahflowKeys => mahflow_keys::unregister_shortcut(app, binding),
    }
}
...
pub fn change_keyboard_implementation_setting(
    ...
    // Initialize new implementation if needed (MahflowKeys needs state)
    if new_impl == KeyboardImplementation::MahflowKeys {
        if initialize_mahflow_keys_with_rollback(&app)? {
...
pub fn get_keyboard_implementation(app: AppHandle) -> String {
    ...
    match settings.keyboard_implementation {
        ...
        KeyboardImplementation::MahflowKeys => "mahflow_keys".to_string(),
    }
}
...
fn validate_shortcut_for_implementation(
    ...
    match implementation {
        ...
        KeyboardImplementation::MahflowKeys => mahflow_keys::validate_shortcut(raw),
    }
}
...
fn parse_keyboard_implementation(s: &str) -> KeyboardImplementation {
    match s {
        ...
        "mahflow_keys" => KeyboardImplementation::MahflowKeys,
        other => {
            warn!(
...
/// Initialize MahflowKeys if not already initialized, with rollback on failure
fn initialize_mahflow_keys_with_rollback(app: &AppHandle) -> Result<bool, String> {
    if app.try_state::<mahflow_keys::MahflowKeysState>().is_some() {
        return Ok(false); // Already initialized, caller should continue
    }

    if let Err(e) = mahflow_keys::init_shortcuts(app) {
        error!("Failed to initialize MahflowKeys: {}", e);
        // Rollback to Tauri
        let mut settings = settings::get_settings(app);
        settings.keyboard_implementation = KeyboardImplementation::Tauri;
        settings::write_settings(app, settings);
        tauri_impl::init_shortcuts(app);
        return Err(format!(
            "Failed to initialize MahflowKeys: {}. Reverted to Tauri.",
            e
        ));
    }
