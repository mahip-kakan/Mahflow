//! Mahflow-keys based keyboard shortcut implementation
//!
//! This module provides an alternative to Tauri's global-shortcut plugin
//! using the mahflow-keys library for more control over keyboard events.
//!
//! ## Architecture
//!
...
/// State for the mahflow-keys shortcut manager
pub struct MahflowKeysState {
...
impl MahflowKeysState {
    /// Create a new MahflowKeysState
    ...
    fn manager_thread(cmd_rx: Receiver<ManagerCommand>, app: AppHandle) {
        info!("mahflow-keys manager thread started");
        ...
        loop {
            ...
            while let Some(event) = manager.try_recv() {
                if let Some((binding_id, hotkey_string)) = hotkey_to_binding.get(&event.id) {
                    debug!(
                        "mahflow-keys event: binding={}, hotkey={}, state={:?}",
            ...
            match cmd_rx.recv_timeout(std::time::Duration::from_millis(10)) {
                Ok(cmd) => match cmd {
                    ...
                    ManagerCommand::Shutdown => {
                        info!("mahflow-keys manager thread shutting down");
                        break;
                    }
                },
        ...
        info!("mahflow-keys manager thread stopped");
    }
    ...
    fn do_register(
        ...
        debug!(
            "Registered mahflow-keys shortcut: {} -> {:?}",
            binding_id, hotkey
        );
        Ok(())
    }
    ...
    fn do_unregister(
        ...
        if let Some(id) = binding_to_hotkey.remove(binding_id) {
            ...
            debug!("Unregistered mahflow-keys shortcut: {}", binding_id);
        }
        Ok(())
    }
    ...
    pub fn start_recording(&self, app: &AppHandle, binding_id: String) -> Result<(), String> {
        ...
        debug!("Started mahflow-keys recording mode");
        Ok(())
    }
    ...
    fn recording_loop(app: AppHandle, running: Arc<AtomicBool>) {
        while running.load(Ordering::SeqCst) {
            let event = {
                let state = match app.try_state::<MahflowKeysState>() {
                    Some(s) => s,
                    None => break,
                };
            ...
            if let Some(key_event) = event {
                ...
                // Emit to frontend
                if let Err(e) = app.emit("mahflow-keys-event", &frontend_event) {
    ...
    pub fn stop_recording(&self) -> Result<(), String> {
        ...
        debug!("Stopped mahflow-keys recording mode");
        Ok(())
    }
}

impl Drop for MahflowKeysState {
    fn drop(&mut self) {
...
/// Convert mahflow-keys Modifiers to a list of strings
...
/// Validate a shortcut string for the MahflowKeys implementation.
/// MahflowKeys is more permissive: allows modifier-only combos and the fn key.
pub fn validate_shortcut(raw: &str) -> Result<(), String> {
    if raw.trim().is_empty() {
        return Err("Shortcut cannot be empty".into());
    }
    // MahflowKeys accepts modifier-only, key-only, and modifier+key combos
    // Just verify the string is parseable
    raw.parse::<Hotkey>()
        .map(|_| ())
        .map_err(|e| format!("Invalid shortcut for MahflowKeys: {}", e))
}

/// Initialize mahflow-keys shortcuts
pub fn init_shortcuts(app: &AppHandle) -> Result<(), String> {
    let state = MahflowKeysState::new(app.clone())?;
    ...
    for (id, default_binding) in default_bindings {
        ...
        if let Err(e) = state.register(&binding) {
            error!(
                "Failed to register mahflow-keys shortcut {} during init: {}",
                id, e
            );
        }
    }
    ...
    app.manage(state);
    info!("mahflow-keys shortcuts initialized");
    Ok(())
}
    ...
    #[cfg(not(target_os = "linux"))]
    {
        ...
        tauri::async_runtime::spawn(async move {
            if let Some(cancel_binding) = get_settings(&app_clone).bindings.get("cancel").cloned() {
                if let Some(state) = app_clone.try_state::<MahflowKeysState>() {
...
pub fn register_shortcut(app: &AppHandle, binding: ShortcutBinding) -> Result<(), String> {
    let state = app
        .try_state::<MahflowKeysState>()
        .ok_or("MahflowKeysState not initialized")?;
...
#[tauri::command]
#[specta::specta]
pub fn start_mahflow_keys_recording(app: AppHandle, binding_id: String) -> Result<(), String> {
    let settings = get_settings(&app);
    if settings.keyboard_implementation != settings::KeyboardImplementation::MahflowKeys {
        return Err("mahflow-keys is not the active keyboard implementation".into());
    }
...
#[tauri::command]
#[specta::specta]
pub fn stop_mahflow_keys_recording(app: AppHandle) -> Result<(), String> {
    let settings = get_settings(&app);
    if settings.keyboard_implementation != settings::KeyboardImplementation::MahflowKeys {
        return Err("mahflow-keys is not the active keyboard implementation".into());
    }
