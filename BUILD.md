...
This guide covers how to set up the development environment and build Mahflow from source across different platforms.
...
```bash
git clone git@github.com:mahip-kakan/Mahflow.git
cd Mahflow
```
...
The raw binary (`src-tauri/target/release/mahflow`) cannot run standalone — it needs Tauri resource files (tray icons, sounds, VAD model) to be co-located at the expected path.

**Install from the deb bundle** (works on any Linux distro):

```bash
cd /tmp
ar x /path/to/Mahflow/src-tauri/target/release/bundle/deb/Mahflow_*_amd64.deb data.tar.gz
tar xzf data.tar.gz
sudo cp usr/bin/mahflow /usr/bin/
sudo cp -r usr/lib/Mahflow /usr/lib/
sudo cp -r usr/share/icons/hicolor/* /usr/share/icons/hicolor/
sudo cp usr/share/applications/Mahflow.desktop /usr/share/applications/
```

After subsequent rebuilds, only the binary needs re-copying:

```bash
sudo cp src-tauri/target/release/mahflow /usr/bin/
```
...
```
Bundling Mahflow_*_amd64.AppImage
...
~/.cache/tauri/linuxdeploy-x86_64.AppImage --appimage-extract-and-run \
  --appdir Mahflow.AppDir --plugin gtk --output appimage
```
